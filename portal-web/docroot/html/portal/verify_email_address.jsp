...
<aui:form action='<%= themeDisplay.getPathMain() + "/portal/verify_email_address" %>' method="post" name="fm">
	...
	<aui:button-row>
		...
		<c:if test="<%= themeDisplay.isSignedIn() && !user.isEmailAddressVerified() %>">
			<aui:button href='<%= themeDisplay.getPathMain() + "/portal/verify_email_address?cmd=" + Constants.SEND + "&referer=" + HttpUtil.encodeURL(referer) + "&p_l_id=" + layout.getPlid() %>' value="send-new-verification-code" />

			<aui:button href='<%= themeDisplay.getPathMain() + "/portal/update_email_address?referer=" + HttpUtil.encodeURL(referer) + "&p_l_id=" + layout.getPlid() %>' value="change-email-address" />
		</c:if>
	</aui:button-row>
</aui:form>